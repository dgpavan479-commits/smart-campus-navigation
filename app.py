...
def explain_route_decision(
    ...
    explanation.append(
        html.Div(
            """
The algorithm selected this route because
its cumulative traversal cost was lower
...
"""
    ...
    explanation.append(
        html.Div(
            f"Distance: {best_distance} meters"
        )
    )
    explanation.append(
        html.Div(
            f"Estimated Walking Time: "
            f"{stats['time']} minutes"
        )
    )
    explanation.append(
        html.Div(
    ...
    # CHOSEN PATH
    explanation.append(
        html.H3("Chosen Route")
    )
    explanation.append(
        html.Div(
            " → ".join(
                map(str, selected_path)
            )
        )
    )
    explanation.append(html.Br())
    ...
    # ALGORITHM LOGIC
    explanation.append(
        html.H3("Algorithm Logic")
    )
    if algorithm == "Dijkstra":
        explanation.append(
            html.Div(
                """
Dijkstra explores routes in increasing
order of cumulative path cost and guarantees
the globally optimal shortest route.
"""
            )
        )
    elif algorithm == "A*":
        explanation.append(
            html.Div(
                """
A* uses both current travel cost and
estimated remaining distance to guide
search toward the destination efficiently.
"""
            )
        )
        ...
        explanation.append(
            html.Div(
                """
Floyd Warshall computes shortest paths
between all node pairs using dynamic
programming and retrieves the minimum-cost
route.
"""
            )
        )
    ...
    explanation.append(
        html.Div(
            f"""
{algorithm} determined that the selected
route provides the most efficient path
between the chosen locations based on
distance and traversal cost.
"""
        )
    )
...
def draw_map(
    ...
    if dijkstra_path:
        children.append(
            dl.Polyline(
                positions=path_to_coordinates(G, dijkstra_path),
                color="orange",
                weight=8,
            )
        )
    ...
    if floyd_path:
        children.append(
            dl.Polyline(
                positions=path_to_coordinates(G, floyd_path),
                color="blue",
                weight=5,
            )
        )
    ...
    if astar_path:
        children.append(
            dl.Polyline(
                positions=path_to_coordinates(G, astar_path),
                color="green",
                weight=5,
            )
        )
...
def load_graph(_, campus_name):
    ...
    G, graph_type, landmarks, error = build_osm_graph(
        campus_name
    )
    GLOBAL_GRAPH = G
    ...
    return (
        html.Div([
            html.H2(
                f"{campus_name} Loaded Successfully"
            ),
            html.Div(
                f"Nodes: {len(G.nodes)}"
            ),
...
def run_algorithms(
    ...
    if error:
        return (
            draw_map(GLOBAL_GRAPH),
            html.Div(
                f"Routing Error: {error}"
            ),
        )
    # DIJKSTRA
    if button == "btn-dijkstra":
        return (
            draw_map(
                GLOBAL_GRAPH,
